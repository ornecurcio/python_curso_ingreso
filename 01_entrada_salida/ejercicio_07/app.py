...
---
Enunciado:
Al presionar el botón  que corresponde a cada operación 
(suma, resta, multiplicación, y división), 
se deberán obtener los valores contenidos en las cajas de texto 
(txtOperadorA y txtOperadorB), 
transformarlos en números enteros, 
realizar dicha operación y luego mostrar el resultado 
...
class App(customtkinter.CTk):
    ...
    def btn_sumar_on_click(self):
        ...
        suma = int(operador_a_texto)+int(operador_b_texto)
    ...
    def btn_restar_on_click(self):
        operador_a_texto = self.txt_operador_a.get()
        operador_b_texto = self.txt_operador_b.get()
        resta = int(operador_a_texto)-int(operador_b_texto)
    ...
    def btn_multiplicar_on_click(self):
        operador_a_texto = self.txt_operador_a.get()
        operador_b_texto = self.txt_operador_b.get()
        multiplicacion = int(operador_a_texto)*int(operador_b_texto)
        mensaje = "El resultado de la multiplicación es: " + str(multiplicacion)
        alert(title="Resultado", message=mensaje)

    def btn_dividir_on_click(self):
        operador_a = self.txt_operador_a.get()
        operador_b = self.txt_operador_b.get()
        division = int(operador_a)/int(operador_b)
        mensaje = "El resultado de la división es: {0}".format(division)
        alert(title="Resultado", message=mensaje)
