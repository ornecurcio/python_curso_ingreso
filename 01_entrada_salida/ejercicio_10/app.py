...
transformarlos en números y mostrar el importe actualizado con 
el descuento utilizando el Dialog Alert.
'''
...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        importe = self.txt_sueldo.get()
        descuento = self.txt_descuento.get()
        importe_total = int(importe)-(int(importe) * int(descuento)/100)
        mensaje = "El importe total con {0}%, es de {1}".format(descuento,importe_total)
